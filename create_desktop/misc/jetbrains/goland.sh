...
GOLAND_JDK=/usr/pkg/java/openjdk17
UNAME=`which uname`
GREP=`which egrep`
GREP_OPTIONS=""
CUT=`which cut`
...
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/GoLand2023.2/goland.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/GoLand2023.2/goland.jdk")
...
if [ -n "$GOLAND_VM_OPTIONS" ] && [ -r "$GOLAND_VM_OPTIONS" ]; then
  ...
  if [ -r "${IDE_BIN_HOME}/goland64.vmoptions" ]; then
    VM_OPTIONS_FILE="${IDE_BIN_HOME}/goland64.vmoptions"
  else
    test "${OS_TYPE}" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
    if [ -r "${IDE_BIN_HOME}/${OS_SPECIFIC}/goland64.vmoptions" ]; then
      VM_OPTIONS_FILE="${IDE_BIN_HOME}/${OS_SPECIFIC}/goland64.vmoptions"
    fi
  fi
  ...
  elif [ -r "${CONFIG_HOME}/JetBrains/GoLand2023.2/goland64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/GoLand2023.2/goland64.vmoptions"
  fi
fi
